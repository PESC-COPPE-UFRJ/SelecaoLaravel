...
<!-- <form class="form-horizontal ng-pristine ng-valid" role="form" method="POST" action="{{URL::to("perfil")}}"> -->

	{{Form::model($perfil, array('route' => array('adm.perfil.update', $perfil->id), 'method' => 'put'))}}

    @include('elements.alerts')

    <section class="panel panel-default">
        <div class="panel-heading"><strong><span class="glyphicon glyphicon-th"></span> Editar Perfil</strong></div>
            ...
            <div class="wizard clearfix" id="steps-uid-0"><div class="steps clearfix"><ul><li id="step0_tab" role="tab" class="first current"><a id="steps-uid-0-t-0"><span class="current-info audible">current step: </span><span class="number">1.</span> Nome do perfil</a></li><li id="step1_tab" role="tab" class="done"><a id="steps-uid-0-t-1"><span class="number">2.</span> Controle de Acesso</a></li></ul></div><div class="content clearfix">
                ...
                <div id="step0" class="current" style="display: block;">
                    ...
                    <section class="panel panel-default">
                        ...
                        <h4>Você esta editando um perfil de acesso ao sistema. Qual o nome do perfil?</h4>
                         
                        {{Form::text('nome', null, array('class' => 'form-control', 'placeholder' => 'Ex: Diretor1, Gerente1, Assistente2'))}}

                        <!-- <input type="text" name="perfil_name" class="form-control" placeholder="Ex: Diretor1, Gerente1, Assistente2"> -->
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    @foreach ($menus as $menu)
                                        ...
                                        <tr class="info">
                                            ...
                                            <td><input id="{{$menu->id}}" class="parent" type="checkbox" name="menus[]" value="{{$menu->id}}" {{$checked}}></td>
                                            <?php $checked = ''; ?>
                                            <td></td>
                                        ...
                                        @foreach($menu->submenus as $submenu)
                                        	@foreach($perfil->menus as $psubmenu)
                                        		@if($submenu->id == $psubmenu->id)
                                        			<?php $checked = 'checked'; ?>
	    										@endif
                                        	@endforeach
                                        ...
                                        <tr class="{{$menu->id}}" style="display: none;">
                                            ...
                                            <td>{{$submenu->nome}}</td>
                                            <td></td>
                                            <td><input id="{{$menu->id}}" class="child" type="checkbox" name="menus[]" value="{{$submenu->id}}" {{$checked}}></td>
                                        </tr>
                                        <?php $checked = ''; ?>
                                        @endforeach
