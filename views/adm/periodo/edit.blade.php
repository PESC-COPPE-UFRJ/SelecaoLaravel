...
$(document).ready(function()
{
    ...
    /* Adiciona prova de MESTRADO ao clicar em nova prova */
        ...
        @foreach($periodo->areas_mestrado_get as $area_mestrado)
            @if(count($area_mestrado->provas) > 0)
                areas_mestrado[{{$area_mestrado->area_id}}] = {{count($area_mestrado->provas) - 1}};
            @endif
        @endforeach
    ...
    @foreach($periodo->areas_doutorado_get as $area_doutorado)
        @if(count($area_doutorado->provas) > 0)
            areas_doutorado[{{$area_doutorado->area_id}}] = {{count($area_doutorado->provas) - 1}};
        @endif
    @endforeach
                ...
                <div id="step0" role="tabpanel" aria-labelledby="steps-uid-0-h-2" class="body current" style="display: block;">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2" id="div_habilitado">
                            <input name="status" type="radio" value="0" @if($periodo->status == 0) checked="" @endif> Inativo
                        </div>
                        <div class="col-sm-2" id="div_habilitado">
                            <input name="status" type="radio" value="1" @if($periodo->status == 1) checked="" @endif> Em Preparação
                        </div>
                        <div class="col-sm-2" id="div_habilitado">
                            <input name="status" type="radio" value="2" @if($periodo->status == 2) checked="" @endif> Inscrições Abertas
                        </div>
                        <div class="col-sm-2" id="div_habilitado">
                            <input name="status" type="radio" value="3" @if($periodo->status == 3) checked="" @endif> Inscrições Concluidas
                        </div>
                        <div class="col-sm-2" id="div_habilitado">
                            <input name="status" type="radio" value="4" @if($periodo->status == 4) checked="" @endif> Periodo Fechado
                        </div>
                    </div>
                    ...
                    @foreach($areas as $area)
                        <div class="form-group clearfix">
                            ...
                            <div class="col-md-5" id="div_habilitado">
                                <input name="areas_vagas_mestrado[{{$area->id}}]" type="text" class="form-control" placeholder="Numero de Vagas Ex: 4" value="{{$areas_mestrado[$area->id]->num_vagas or 0}}">
                            </div>
                            <div class="col-sm-3">
                            ...
                            <div class="col-md-12 divprovaconfig" id="divprova{{$area->sigla}}mestrado" style="display:none;">
                                ...
                                @foreach($periodo->areas_mestrado_get as $area_mestrado)
                                    {? $i = 0; ?}
                                    @if($area_mestrado->area_id == $area->id)
                                        @foreach($area_mestrado->provas as $prova_mestrado)
                                        <input type="hidden" name="provas_mestrado[{{$prova_mestrado->area_id}}][{{$i}}][id]" value="{{$prova_mestrado->id}}" class="div_prova_mestrado_{{$prova_mestrado->area_id}}_{{$i}}" />
                                        <div class="form-group div_prova_mestrado_{{$prova_mestrado->area_id}}_{{$i}}">
                                            <div class="row">
                                                <label class="col-md-1 control-label"><button type="button" class="btn btn-danger prova_mestrado" areaid="{{$prova_mestrado->area_id}}" prova_i="{{$i}}" prova_id="{{$prova_mestrado->id}}" periodo_id="{{$periodo->id}}" areasigla="BD">Deletar</button></label>
                                                <label class="col-md-2 control-label">Identificador:</label>
                                                <div class="col-md-3">           <input type="text" name="provas_mestrado[{{$prova_mestrado->area_id}}][{{$i}}][identificador]" placeholder="Identificador" class="form-control input-md" value="{{$prova_mestrado->identificador}}">           <span class="help-block">Ex: P1 ou P2...</span>       </div>
                                                <label class="col-md-3 control-label" for="selectbasic">Nome da Prova:</label>
                                                <div class="col-md-3">
                                                    <select id="selectbasic" name="provas_mestrado[{{$prova_mestrado->area_id}}][{{$i}}][id_prova]" class="form-control">
                                                        @foreach($provas as $key => $prova)
                                                            <option value="{{$key}}" @if($prova_mestrado->prova_id == $key) selected="" @endif>{{$prova}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <label class="col-md-3 control-label" for="selectbasic">Tipo da Prova:</label>
                                                <div class="col-md-3">
                                                    <select id="selectbasic" areaid="{{$prova_mestrado->area_id}}" prova_i="{{$i}}" name="provas_mestrado[{{$prova_mestrado->area_id}}][{{$i}}][tipo]" class="form-control tipo_mestrado">
                                                        <option>Selecione uma opção</option>
                                                        <option value="Eliminatoria" @if($prova_mestrado->tipo == "Eliminatoria") selected="" @endif>Eliminatória</option>
                                                        <option value="Classificatoria" @if($prova_mestrado->tipo == "Classificatoria") selected="" @endif>Classificatória</option>
                                                    </select>
                                                </div>
                                                <label class="col-md-3 control-label tipo_mestrado_eliminatoria{{$prova_mestrado->area_id}}-{{$i}}" for="selectbasic" @if($prova_mestrado->tipo == 'Classificatoria') style="display: none;" @endif>Nota Eliminatória:</label>
                                                <div class="col-md-3 tipo_mestrado_eliminatoria{{$prova_mestrado->area_id}}-{{$i}}" @if($prova_mestrado->tipo == 'Classificatoria') style="display: none;" @endif>
                                                    <select name="provas_mestrado[{{$prova_mestrado->area_id}}][{{$i}}][nota_eliminatoria]" class="form-control">
                                                        <option value="1" @if($prova_mestrado->nota_eliminatoria == 1) selected="" @endif>1</option>
                                                        <option value="2" @if($prova_mestrado->nota_eliminatoria == 2) selected="" @endif>2</option>
                                                        <option value="3" @if($prova_mestrado->nota_eliminatoria == 3) selected="" @endif>3</option>
                                                        <option value="4" @if($prova_mestrado->nota_eliminatoria == 4) selected="" @endif>4</option>
                                                        <option value="5" @if($prova_mestrado->nota_eliminatoria == 5) selected="" @endif>5</option>
                                                        <option value="6" @if($prova_mestrado->nota_eliminatoria == 6) selected="" @endif>6</option>
                                                        <option value="7" @if($prova_mestrado->nota_eliminatoria == 7) selected="" @endif>7</option>
                                                        <option value="8" @if($prova_mestrado->nota_eliminatoria == 8) selected="" @endif>8</option>
                                                        <option value="9" @if($prova_mestrado->nota_eliminatoria == 9) selected="" @endif>9</option>
                                                        <option value="10" @if($prova_mestrado->nota_eliminatoria == 10) selected="" @endif>10</option>
                                                    </select>
                                                </div>
                                                <label class="col-md-3 control-label tipo_mestrado_classificatoria{{$prova_mestrado->area_id}}-{{$i}}" for="selectbasic" @if($prova_mestrado->tipo == 'Eliminatoria') style="display: none;" @endif>Nota Classificatória:</label>
                                                <div class="col-md-3 tipo_mestrado_classificatoria{{$prova_mestrado->area_id}}-{{$i}}" @if($prova_mestrado->tipo == 'Eliminatoria') style="display: none;" @endif>
                                                    <select name="provas_mestrado[{{$prova_mestrado->area_id}}][{{$i}}][nota_classificatoria]" class="form-control">
                                                        <option value="1" @if($prova_mestrado->nota_classificatoria == 1) selected="" @endif>1</option>
                                                        <option value="2" @if($prova_mestrado->nota_classificatoria == 2) selected="" @endif>2</option>
                                                        <option value="3" @if($prova_mestrado->nota_classificatoria == 3) selected="" @endif>3</option>
                                                        <option value="4" @if($prova_mestrado->nota_classificatoria == 4) selected="" @endif>4</option>
                                                        <option value="5" @if($prova_mestrado->nota_classificatoria == 5) selected="" @endif>5</option>
                                                        <option value="6" @if($prova_mestrado->nota_classificatoria == 6) selected="" @endif>6</option>
                                                        <option value="7" @if($prova_mestrado->nota_classificatoria == 7) selected="" @endif>7</option>
                                                        <option value="8" @if($prova_mestrado->nota_classificatoria == 8) selected="" @endif>8</option>
                                                        <option value="9" @if($prova_mestrado->nota_classificatoria == 9) selected="" @endif>9</option>
                                                        <option value="10" @if($prova_mestrado->nota_classificatoria == 10) selected="" @endif>10</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        <hr style="border-top: 1px solid #ccc;" class="div_prova_mestrado_{{$prova_mestrado->area_id}}_{{$i}}">
                                        {? $i++; ?}
                                        @endforeach
                                    @endif
                                @endforeach
                ...
                <div id="step2" role="tabpanel" aria-labelledby="steps-uid-0-h-2" class="body" style="display: block;">
                    ...
                    @foreach($areas as $area)
                        <div class="form-group clearfix">
                            <label for="habilitado" class="col-md-4 control-label" style="text-align: right;">{{$area->nome}} ({{$area->sigla}})</label>
                            <div class="col-md-5" id="div_habilitado">
                                <input name="areas_vagas_doutorado[{{$area->id}}]" type="text" class="form-control" placeholder="Numero de Vagas Ex: 4" value="{{$areas_doutorado[$area->id]->num_vagas or 0}}">
                            </div>
                            <div class="col-sm-3">
                            ...
                            <div class="col-md-12 divprovaconfig" id="divprova{{$area->sigla}}doutorado" style="display:none;">
                                ...
                                @foreach($periodo->areas_doutorado_get as $area_doutorado)
                                    {? $i = 0; ?}
                                    @if($area_doutorado->area_id == $area->id)
                                        @foreach($area_doutorado->provas as $prova_doutorado)
                                        <input type="hidden" name="provas_doutorado[{{$prova_doutorado->area_id}}][{{$i}}][id]" value="{{$prova_doutorado->id}}" class="div_prova_doutorado_{{$prova_doutorado->area_id}}_{{$i}}" />
                                        <div class="form-group div_prova_doutorado_{{$prova_doutorado->area_id}}_{{$i}}">
                                            <div class="row">
                                                <label class="col-md-1 control-label"><button type="button" class="btn btn-danger prova_doutorado" areaid="{{$prova_doutorado->area_id}}" prova_i="{{$i}}" prova_id="{{$prova_doutorado->id}}" periodo_id="{{$periodo->id}}" areasigla="BD">Deletar</button></label>
                                                <label class="col-md-2 control-label">Identificador:</label>
                                                <div class="col-md-3">           <input type="text" name="provas_doutorado[{{$prova_doutorado->area_id}}][{{$i}}][identificador]" placeholder="Identificador" class="form-control input-md" value="{{$prova_doutorado->identificador}}">           <span class="help-block">Ex: P1 ou P2...</span>       </div>
                                                <label class="col-md-3 control-label" for="selectbasic">Nome da Prova:</label>
                                                <div class="col-md-3">
                                                    <select id="selectbasic" name="provas_doutorado[{{$prova_doutorado->area_id}}][{{$i}}][id_prova]" class="form-control">
                                                        @foreach($provas as $key => $prova)
                                                            <option value="{{$key}}" @if($prova_doutorado->prova_id == $key) selected="" @endif>{{$prova}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <label class="col-md-3 control-label" for="selectbasic">Tipo da Prova:</label>
                                                <div class="col-md-3">
                                                    <select id="selectbasic" areaid="{{$prova_doutorado->area_id}}" prova_i="{{$i}}" name="provas_doutorado[{{$prova_doutorado->area_id}}][{{$i}}][tipo]" class="form-control tipo_doutorado">
                                                        <option>Selecione uma opção</option>
                                                        <option value="Eliminatoria" @if($prova_doutorado->tipo == "Eliminatoria") selected="" @endif>Eliminatória</option>
                                                        <option value="Classificatoria" @if($prova_doutorado->tipo == "Classificatoria") selected="" @endif>Classificatória</option>
                                                    </select>
                                                </div>
                                                <label class="col-md-3 control-label tipo_doutorado_eliminatoria{{$prova_doutorado->area_id}}-{{$i}}" for="selectbasic" @if($prova_doutorado->tipo == 'Classificatoria') style="display: none;" @endif>Nota Eliminatória:</label>
                                                <div class="col-md-3 tipo_doutorado_eliminatoria{{$prova_doutorado->area_id}}-{{$i}}" @if($prova_doutorado->tipo == 'Classificatoria') style="display: none;" @endif>
                                                    <select name="provas_doutorado[{{$prova_doutorado->area_id}}][{{$i}}][nota_eliminatoria]" class="form-control">
                                                        <option value="1" @if($prova_doutorado->nota_eliminatoria == 1) selected="" @endif>1</option>
                                                        <option value="2" @if($prova_doutorado->nota_eliminatoria == 2) selected="" @endif>2</option>
                                                        <option value="3" @if($prova_doutorado->nota_eliminatoria == 3) selected="" @endif>3</option>
                                                        <option value="4" @if($prova_doutorado->nota_eliminatoria == 4) selected="" @endif>4</option>
                                                        <option value="5" @if($prova_doutorado->nota_eliminatoria == 5) selected="" @endif>5</option>
                                                        <option value="6" @if($prova_doutorado->nota_eliminatoria == 6) selected="" @endif>6</option>
                                                        <option value="7" @if($prova_doutorado->nota_eliminatoria == 7) selected="" @endif>7</option>
                                                        <option value="8" @if($prova_doutorado->nota_eliminatoria == 8) selected="" @endif>8</option>
                                                        <option value="9" @if($prova_doutorado->nota_eliminatoria == 9) selected="" @endif>9</option>
                                                        <option value="10" @if($prova_doutorado->nota_eliminatoria == 10) selected="" @endif>10</option>
                                                    </select>
                                                </div>
                                                <label class="col-md-3 control-label tipo_doutorado_classificatoria{{$prova_doutorado->area_id}}-{{$i}}" for="selectbasic" @if($prova_doutorado->tipo == 'Eliminatoria') style="display: none;" @endif>Nota Classificatória:</label>
                                                <div class="col-md-3 tipo_doutorado_classificatoria{{$prova_doutorado->area_id}}-{{$i}}" @if($prova_doutorado->tipo == 'Eliminatoria') style="display: none;" @endif>
                                                    <select name="provas_doutorado[{{$prova_doutorado->area_id}}][{{$i}}][nota_classificatoria]" class="form-control">
                                                        <option value="1" @if($prova_doutorado->nota_classificatoria == 1) selected="" @endif>1</option>
                                                        <option value="2" @if($prova_doutorado->nota_classificatoria == 2) selected="" @endif>2</option>
                                                        <option value="3" @if($prova_doutorado->nota_classificatoria == 3) selected="" @endif>3</option>
                                                        <option value="4" @if($prova_doutorado->nota_classificatoria == 4) selected="" @endif>4</option>
                                                        <option value="5" @if($prova_doutorado->nota_classificatoria == 5) selected="" @endif>5</option>
                                                        <option value="6" @if($prova_doutorado->nota_classificatoria == 6) selected="" @endif>6</option>
                                                        <option value="7" @if($prova_doutorado->nota_classificatoria == 7) selected="" @endif>7</option>
                                                        <option value="8" @if($prova_doutorado->nota_classificatoria == 8) selected="" @endif>8</option>
                                                        <option value="9" @if($prova_doutorado->nota_classificatoria == 9) selected="" @endif>9</option>
                                                        <option value="10" @if($prova_doutorado->nota_classificatoria == 10) selected="" @endif>10</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        <hr style="border-top: 1px solid #ccc;" class="div_prova_doutorado_{{$prova_doutorado->area_id}}_{{$i}}">
...
<script type="text/javascript">
    $(function() { 
        $("#title_text").html("Editando Período {{$periodo->ano}}.{{$periodo->periodo}}");

        $("#ano").change(function(){
            var ano = $(this).val();
            var periodo = $("#periodo").val();
            if(ano != ''){
                $("#title_text").html("Editando Período "+ano+"."+periodo);
            } else {
                $("#title_text").html("Editando Período");
            }
        });
        ...
        $("#steps-uid-0-t-1").click(function(){
            ...
            var periodo = $("#periodo").val();
            if(ano != ''){
                $("#title_text").html("Editando Período "+ano+"."+periodo+" - Mestrado");
            } else {
                $("#title_text").html("Editando Período - Mestrado");
            }
        });
        $("#steps-uid-0-t-2").click(function(){
            ...
            var periodo = $("#periodo").val();
            if(ano != ''){
                $("#title_text").html("Editando Período "+ano+"."+periodo+" - Doutorado");
            } else {
                $("#title_text").html("Editando Período - Doutorado");
            }
        });
        $("#next").click(function(){
            var ano = $("#ano").val();
